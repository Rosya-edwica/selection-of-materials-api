...
async def save_playlists_to_history(db, skill: str, playlists: list[PlayList]):
    await db.executemany(f"INSERT INTO playlist(id, name, url, img) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING;",
                         [(playlist.id, playlist.name.replace("'", "`").replace('"', '`'), playlist.link, playlist.header_image)
...
async def set_connect_between_playlists_and_skill(db, skill: str, playlists_ids: list[str]):
    await db.executemany(f"INSERT INTO skill_to_playlist(skill, playlist_id) VALUES($1, $2) ON CONFLICT DO NOTHING;",
                         [(skill, playlist_id) for playlist_id in playlists_ids])


async def find_playlists_in_history(db, skill: str, limit: int) -> SkillPlaylists | None:
