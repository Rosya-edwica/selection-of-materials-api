...
async def set_connect_between_videos_and_skill(db, skill: str, video_ids: list[str]):
    await db.executemany(f"INSERT INTO skill_to_video(skill, video_id) VALUES($1, $2) ON CONFLICT DO NOTHING;", [(skill, video_id) for video_id in video_ids])
    
async def find_videos_in_history(db, skill: str, limit: int) -> list[SkillVideos] | None:
